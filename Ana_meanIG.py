...
while m < len(MEM):	# m < 20
	...
	if exp=='/GFS':
		...
		# ------Geopotencial------	
		geopt_M[:,:,:,m] = np.ma.load(path_upp + '/' + MEM[m] + '/geopt_upp_' + YY + MM + DD + HH + '_ana' + ANA )	
		# ------Temperatura------
		tk_M[:,:,:,m]= np.ma.load(path_upp + '/' + MEM[m] + '/tk_upp_' + YY + MM + DD + HH +  '_ana' + ANA  )
		# ------Humedad especifica------
		q_M[:,:,:,m]= np.ma.load(path_upp + '/' + MEM[m] + '/q_upp_' + YY + MM + DD + HH +  '_ana' + ANA  )
		# ------Viento: componentes U y V------
		u_M[:,:,:,m]= np.ma.load(path_upp + '/' + MEM[m] + '/u_upp_' + YY + MM + DD + HH +  '_ana' + ANA  )
		v_M[:,:,:,m]= np.ma.load(path_upp + '/' + MEM[m] + '/v_upp_'+ YY + MM + DD + HH +  '_ana' + ANA  )
		# ------PSFC------
		psfc_M[:,:,m]= np.ma.load(path_upp + '/' + MEM[m] + '/psfc_upp_'+ YY + MM + DD + HH +  '_ana' + ANA  )
...
np.ma.dump(geopt_mean, path_salida  + '/geopt_mean_' + YY + MM + DD + HH +  periodo +'_ana' + ANA  )
np.ma.dump(tk_mean, path_salida  + '/tk_mean_'+ YY + MM + DD + HH +  periodo +'_ana' + ANA  )
np.ma.dump(q_mean, path_salida  + '/q_mean_' + YY + MM + DD + HH +  periodo +'_ana' + ANA  )
np.ma.dump(u_mean, path_salida  + '/u_mean_' + YY + MM + DD + HH +  periodo +'_ana' + ANA )
np.ma.dump(v_mean, path_salida + '/v_mean_' + YY + MM + DD + HH +  periodo +'_ana' + ANA  )
np.ma.dump(psfc_mean, path_salida  + '/psfc_mean_' + YY + MM + DD + HH +  periodo +'_ana' + ANA  )

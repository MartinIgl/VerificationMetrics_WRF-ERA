...
while m < len(M):	# m < 20
	...
	# ------Geopotencial------	
	geopt_upp = np.ma.load(path_upp + '/' + M[m] + '/geopt_upp_' + YY + MM + DD + HH + FCST )
	geopt_era = np.load(path_eralvl + '/geopt_era_' + YY + MM + DD + HH + '.npy')
	dif_geopt = geopt_era - geopt_upp

	# ------Temperatura------
        tk_upp = np.ma.load(path_upp + '/' + M[m] + '/tk_upp_' + YY + MM + DD + HH + FCST )
        tk_era = np.load(path_eralvl + '/tk_era_' + YY + MM + DD + HH + '.npy')
        dif_tk = tk_era - tk_upp
	# ------Humedad especifica------
        q_upp = np.ma.load(path_upp + '/' + M[m] + '/q_upp_' + YY + MM + DD + HH + FCST )
        q_era = np.load(path_eralvl + '/q_era_' + YY + MM + DD + HH + '.npy')
        dif_q = q_era - q_upp
	# ------Viento: componentes U y V------
        u_upp = np.ma.load(path_upp + '/' + M[m] + '/u_upp_' + YY + MM + DD + HH + FCST )
        u_era = np.load(path_eralvl + '/u_era_' + YY + MM + DD + HH + '.npy')
        dif_u = u_era - u_upp

        v_upp = np.ma.load(path_upp + '/' + M[m] + '/v_upp_'+ YY + MM + DD + HH + FCST )
        v_era = np.load(path_eralvl + '/v_era_' + YY + MM + DD + HH + '.npy')
        dif_v = v_era - v_upp
	# ------PSFC------
	psfc_upp = np.ma.load(path_upp + '/' + M[m] + '/psfc_upp_'+ YY + MM + DD + HH + FCST )
	...
	# Solo necesito una subseleccion de lat-lon que incluya la region, no me interesa el globo entero:
        ...
        np.ma.dump(dif_geopt, path_salida + '/' + M[m] + '/geopt_dif_era-upp_' + YY + MM + DD + HH + FCST )
	np.ma.dump(dif_tk, path_salida + '/' + M[m] + '/tk_dif_era-upp_'+ YY + MM + DD + HH + FCST )
        np.ma.dump(dif_q, path_salida + '/' + M[m] + '/q_dif_era-upp_' + YY + MM + DD + HH + FCST )
        np.ma.dump(dif_u, path_salida + '/' + M[m] + '/u_dif_era-upp_' + YY + MM + DD + HH + FCST)
        np.ma.dump(dif_v, path_salida + '/' + M[m] + '/v_dif_era-upp_' + YY + MM + DD + HH + FCST )
        np.ma.dump(dif_psfc, path_salida + '/' + M[m] + '/psfc_dif_era-upp_' + YY + MM + DD + HH + FCST )
